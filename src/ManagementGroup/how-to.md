# How-To
This document describes how to develop for `Az.ManagementGroup`.

## Building `Az.ManagementGroup`
...
There are multiple scripts created for performing different actions for developing `Az.ManagementGroup`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ManagementGroup.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ManagementGroup.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ManagementGroup`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ManagementGroup` into the session.
